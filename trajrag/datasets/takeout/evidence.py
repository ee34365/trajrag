...
import os as _os_mt
_MAXTOK = int(_os_mt.environ.get("TRAJRAG_MAXTOK_SRC", "500"))
...
def retrieve_point(uid: str, args: dict, conn, profile_text: str = "") -> dict | None:
    ...
    if skip == 0 and (anchor == 'home' or anchor == 'work'):
        try:
            from anchor_v2_module import detect_from_traj, anchor_from_cluster
            raw_traj = _RAW_TRAJ_CACHE.get(str(uid))
            if raw_traj:
                _av2 = detect_from_traj(raw_traj)
                if anchor == 'home' and _av2.get('home'):
                    a = anchor_from_cluster(_av2['home'], 'home')
                    print(f"[retrieve_point] anchor_v2 HOME for uid={uid}: {a['anchor_name']} @({a['lat']:.5f},{a['lon']:.5f}) night_h={_av2['home']['night_h']}")
                    return a
                if anchor == 'work' and _av2.get('work'):
                    w = _av2['work']
                    if not (exclude_poi_id and w.get('top_pid') and (w['top_pid'] == exclude_poi_id or w['top_pid'].endswith(exclude_poi_id) or exclude_poi_id.endswith(w['top_pid']))):
                        a = anchor_from_cluster(w, 'work')
                        print(f"[retrieve_point] anchor_v2 WORK for uid={uid}: {a['anchor_name']} @({a['lat']:.5f},{a['lon']:.5f}) work_h={w['work_h']}")
                        return a
        except Exception as _e:
            print(f'[retrieve_point] anchor_v2 error uid={uid}: {_e}')
    ...
    if anchor == "home":
        ...
        goal = f"Get the most recent visit location of user '{uid}'. Return poi_id, poi_name, d_lat AS lat, d_lon AS lon."
        import os as _os_co
        if _os_co.environ.get("TRAJRAG_CURRENT_ORIGIN"):
            fallback = f"""
                SELECT poi_id, poi_name, o_lat AS lat, o_lon AS lon
                FROM trajectory
                WHERE uid = '{uid}' AND status = 'activity'
    ...
    import os as _os_fc
    if anchor == "current" and not _os_fc.environ.get("TRAJRAG_LEAKY_CURRENT"):
        primary = ""
    if anchor == "current":
        _one = " ".join(str(primary or "").split())
        print(f"  [ANCHOR_SQL_LOG] src={plan.get('source')} sql={_one[:400]}", flush=True)
...
def retrieve_route(uid: str, args: dict, conn, profile_text: str = "") -> dict | None:
    ...
    home = retrieve_point(uid, {"anchor": "home"}, conn, profile_text)
    if home is None:
        return None

    work = retrieve_point(uid, {"anchor": "work", "exclude_poi_id": home.get("poi_id")}, conn, profile_text)
    if work is None:
        return None

    if haversine(home["lat"], home["lon"], work["lat"], work["lon"]) < 0.5:
        work2 = retrieve_point(uid, {"anchor": "work", "exclude_poi_id": home.get("poi_id"), "skip": 1}, conn, profile_text)
        if work2 is None or haversine(home["lat"], home["lon"], work2["lat"], work2["lon"]) < 0.5:
            print(f"[retrieve_route] home==work for uid={uid}; fallback to single-point at home")
            return {"route_name": direction,
                    "points": [{"lat": home["lat"], "lon": home["lon"], "timestamp": None}],
                    "source": "route_fallback_single_point_home",
                    "home": home, "work": home}
        work = work2
...
def _llm_write_spatial_sql(intent: dict, R_tau: dict, cat_aliases: list, top_k: int) -> str:
    ...
    try:
        rsp = _client.chat.completions.create(
            **_NO_REASON_KW,
            model=LLM_MODEL,
            messages=[
                {"role": "system", "content": _SPATIAL_PLAN_SYSTEM},
    ...
    import os as _os_cc
    if _os_cc.environ.get("TRAJRAG_CAT_CAP") and cat_aliases:
        _allowed = {str(c).lower().strip() for c in cat_aliases}
        _kept = [c for c in whitelist if c in _allowed]
        if _kept:
            if len(_kept) != len(whitelist):
                print(f"  [POI cand X] whitelist {len(whitelist)} -> {len(_kept)} (capped to alias set)")
            whitelist = _kept
    ...
    _cap_geom(
        ...
        polygon_wkt=polygon_wkt_full,
        mls_wkt=mls_wkt_full,
...
ABLATE_STATS = {"total": 0, "llm_ok": 0, "gen_fail": 0, "exec_fail": 0,
                "invalid": 0, "empty": 0}

_SCHEMA_DDL = """Tables (DuckDB, spatial extension loaded):

  pois(poi_id TEXT, name TEXT, primary_type TEXT, lat DOUBLE, lon DOUBLE,
       address TEXT, opening_hours TEXT)

  trajectory(uid TEXT, event_start TIMESTAMP, event_end TIMESTAMP,
             poi_id TEXT, poi_name TEXT, poi_type TEXT,
             lat DOUBLE, lon DOUBLE,
             o_lat DOUBLE, o_lon DOUBLE, d_lat DOUBLE, d_lon DOUBLE,
             status TEXT)      -- status is 'visit' or 'move'
"""

_REQUIRED_OUT = ("The SELECT must return exactly these columns, in this order:\n"
                 "  poi_id, name, category, lat, lon, address, opening_hours, dist_km\n"
                 "where category = primary_type and dist_km is a DOUBLE distance in km "
                 "(use 0.0 if no distance applies). Return ONLY the SQL, no markdown, "
                 "no commentary, no trailing semicolon.")


def _llm_raw_spatial_sql(intent, R_tau, cat_aliases, top_k, with_evidence=True):
    query_text = intent.get("query") or ""
    mode = (intent.get("spatial") or {}).get("mode") or "none"
    parts = [_SCHEMA_DDL, "", f'User query: "{query_text[:300]}"',
             f'Detected spatial mode: {mode}']
    if cat_aliases:
        parts.append(f"Candidate POI categories (primary_type): {[str(a).lower() for a in cat_aliases]}")

    if with_evidence:
        z = R_tau.get("zone") or {}
        r = R_tau.get("route") or {}
        p = R_tau.get("point") or {}
        if mode == "zone" and z.get("polygon_wkt"):
            parts.append(f'Zone name: {z.get("zone_name","?")}')
            parts.append("Zone polygon (WKT), use ST_Contains / ST_GeomFromText:")
            parts.append(z["polygon_wkt"])
        elif mode == "route":
            direction = r.get("route_name", "home_to_work")
            chains = (r.get("chains_to_work") if direction == "home_to_work"
                      else r.get("chains_to_home")) or r.get("chains_to_home") or r.get("chains_to_work")
            lss = []
            for c in (chains or []):
                coords = [(float(pt["lat"]), float(pt["lon"])) for pt in c.get("points", [])
                          if pt.get("lat") is not None and pt.get("lon") is not None]
                if len(coords) >= 2:
                    lss.append("(" + ", ".join(f"{lo} {la}" for la, lo in coords) + ")")
            if lss:
                parts.append(f"Commute direction: {direction}")
                parts.append("Commute route (WKT MULTILINESTRING), use ST_Distance / ST_GeomFromText:")
                parts.append("MULTILINESTRING(" + ", ".join(lss) + ")")
        elif mode == "point" and p.get("lat") is not None:
            parts.append(f'Anchor point: lat={p["lat"]}, lon={p["lon"]}  '
                         f'(anchor type: {p.get("anchor_name","current")})')
    else:
        parts.append(
            "No precomputed spatial evidence is provided. Derive whatever you need "
            "(home location, work location, commute route, activity region) directly "
            "from the trajectory table for this user, inside your SQL.")
        uid = intent.get("_uid")
        if uid:
            parts.append(f"User uid = '{uid}'")

    parts.append("")
    parts.append(f"Retrieve at most {top_k} candidate POIs from `pois` that satisfy the query.")
    parts.append(_REQUIRED_OUT)
        ...
        rsp = _client.chat.completions.create(
            **_NO_REASON_KW,
            ...
            messages=[{"role": "system",
                       "content": "You write DuckDB spatial SQL. Output SQL only."},
                      {"role": "user", "content": "\n".join(parts)}],
            temperature=0.0,
        )
        sql = (rsp.choices[0].message.content or "").strip()
    except Exception as e:
        ABLATE_STATS["gen_fail"] += 1
        print(f"  [ABLATE] SQL generation failed: {str(e)[:100]}")
        return ""
    sql = re.sub(r"^```(?:sql)?|```$", "", sql, flags=re.M).strip().rstrip(";")
    if not sql.lower().startswith("select"):
        ABLATE_STATS["gen_fail"] += 1
        print(f"  [ABLATE] output is not a SELECT: {sql[:80]}")
        return ""
    return sql


def _ablate_retrieve(intent, R_tau, conn, cat_aliases, top_k, with_evidence):
    ABLATE_STATS["total"] += 1
    sql = _llm_raw_spatial_sql(intent, R_tau, cat_aliases, top_k, with_evidence)
    if not sql:
        return []
    try:
        df = conn.execute(sql).fetchdf()
    except Exception as e:
        ABLATE_STATS["exec_fail"] += 1
        print(f"  [ABLATE] SQL execution failed: {str(e)[:110]}")
        return []
    need = ["poi_id", "name", "category", "lat", "lon", "dist_km"]
    if df is None or any(c not in df.columns for c in need):
        ABLATE_STATS["invalid"] += 1
        print(f"  [ABLATE] column mismatch: {list(df.columns) if df is not None else None}")
        return []
    if len(df) == 0:
        ABLATE_STATS["empty"] += 1
        print("  [ABLATE] empty result")
        return []
    ABLATE_STATS["llm_ok"] += 1
    out = []
    for _, row in df.head(top_k).iterrows():
        try:
            out.append({
                "poi_id": str(row["poi_id"]), "name": str(row["name"]),
                "category": str(row["category"]),
                "lat": float(row["lat"]) if pd.notna(row["lat"]) else None,
                "lon": float(row["lon"]) if pd.notna(row["lon"]) else None,
                "address": str(row["address"]) if "address" in df.columns and pd.notna(row["address"]) else "",
                "opening_hours": str(row["opening_hours"]) if "opening_hours" in df.columns and pd.notna(row["opening_hours"]) else "",
                "dist_km": float(row["dist_km"]) if pd.notna(row["dist_km"]) else 0.0,
            })
        except Exception:
            continue
    print(f"  [ABLATE] LLM SQL -> {len(out)} candidates")
    return out


def retrieve_poi_candidates_via_sql(intent: dict, R_tau: dict, conn, top_k: int = 1500) -> list[dict]:
    import os as _os
    _abl = _os.environ.get("TRAJRAG_ABLATE", "").lower()
    if _abl in ("nocompiler", "noevidence"):
        _cats = []
        _tc = (intent.get("target") or {}).get("categories") or []
        if not _tc and (intent.get("target") or {}).get("category"):
            _tc = [intent["target"]["category"]]
        for _c in _tc:
            _lc = str(_c).lower().strip()
            if not _lc:
                continue
            _cur = list(CATEGORY_ALIASES.get(_lc, []))
            if _lc not in [a.lower() for a in _cur]:
                _cur.append(_lc)
            _cats.extend(_cur)
        _seen = set()
        _cats = [c for c in _cats if not (c.lower() in _seen or _seen.add(c.lower()))]
        return _ablate_retrieve(intent, {} if _abl == "noevidence" else R_tau,
                                conn, _cats, top_k,
                                with_evidence=(_abl == "nocompiler"))
    try:
        _spatial_ok_w = False
        try:
                ...
                try:
                    ...
                    if len(df_v) > 0:
                        ...
                        _cap_geom(
                            ...
                            polygon_wkt=polygon_wkt_v if 'polygon_wkt_v' in dir() else polygon_wkt_full,
                    ...
                    try:
                        ...
                        if len(df_v) > 0:
                            ...
                            _cap_geom(
                                ...
                                whitelist=list(cat_aliases_v)[:20] if cat_aliases_v else [],
                                sql=sql_v[:2000],
                                source='patch_v_route',
                                cand_ids=[(c['poi_id'], c.get('lat'), c.get('lon')) for c in out_v[:100]],
                                cand_n=len(out_v),
                                chains_wkt=linestrings,
