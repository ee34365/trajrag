...
_VALID_TYPES_BY_DATASET = {}

def _load_valid_types():
    import pickle, os
    try:
        import evidence as _ev
        ds = _ev.get_active_dataset()
    except Exception:
        ds = "japan"
    if ds in _VALID_TYPES_BY_DATASET:
        return _VALID_TYPES_BY_DATASET[ds]
    fname = "valid_type_embs.pkl" if ds == "japan" else "valid_type_embs_geolife.pkl"
    p_pkl = os.path.join(os.path.dirname(__file__), fname)
    if not os.path.exists(p_pkl):
        p_pkl = os.path.join(os.path.dirname(__file__), "valid_type_embs.pkl")
    if not os.path.exists(p_pkl):
        _VALID_TYPES_BY_DATASET[ds] = ([], None, None)
        return [], None, None
    d = pickle.load(open(p_pkl, "rb"))
    types_ = d["types"]
    embs_ = d["embeddings"].astype(np.float32)
    n = np.linalg.norm(embs_, axis=1)
    n[n < 1e-8] = 1.0
    _VALID_TYPES_BY_DATASET[ds] = (types_, embs_, n)
    global _VALID_TYPES, _VALID_TYPE_EMBS, _VALID_TYPE_NORMS
    _VALID_TYPES = types_
    _VALID_TYPE_EMBS = embs_
    _VALID_TYPE_NORMS = n
    return types_, embs_, n
