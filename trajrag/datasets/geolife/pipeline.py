...
def build_conn_from_traj(uid: str, traj: list,
    ...
    if admin_df is not None and len(admin_df) > 0:
        ...
        try:
            ...
            _sample = adf["name_en"].dropna().astype(str).head(50).tolist() if "name_en" in adf.columns else []
            _joined = " ".join(_sample).lower()
            if any(w in _joined for w in ["subdistrict", "haidian", "sanlitun", "zhong", "beijing", "chaoyang"]):
                _ev.set_active_dataset("china")
                print("[pipeline] auto-detected dataset: china")
            elif any(w in _joined for w in ["ward", "prefecture", "chiyoda", "shibuya", "shinjuku"]):
                _ev.set_active_dataset("japan")
                print("[pipeline] auto-detected dataset: japan")
            else:
                _ev.set_active_dataset("japan")
                print("[pipeline] dataset default: japan")
        except Exception as _e:
            print(f"[pipeline] dataset detect skipped: {_e}")
